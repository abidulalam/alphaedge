  ...
  return (
    <div style={{ maxWidth: 800 }}>
      ...
      {rates.length >= 4 && rates.every(r => r.current != null) && (
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 6, padding: '14px 18px', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12, height: 100 }}>
            {rates.map(r => {
              const maxRate = Math.max(...rates.map(x => x.current ?? 0)) || 1
      ...
      {rates.map(r => {
        ...
        // label every ~13 weeks (quarterly)
                  ...
                  <div style={{ maxWidth: 700 }}>
                    ...
                    <div style={{ border: '1px solid var(--border)', borderRadius: 6, overflow: 'hidden', marginBottom: 4 }}>
                      ...
                      {[
                        ...
                        { l: 'Free Cash Flow (TTM)', v: data.freeCashflow ? fmtNum(data.freeCashflow) : null, i: data.freeCashflow ? (data.freeCashflow > 0 ? 'Positive — shareholder value' : 'Negative — cash burn') : null },
                ...
                {tab === 'AI Report' && (
                  <div style={{ maxWidth: 680 }}>
                    ...
                    {[
                      ...
                      { h: 'Profitability & Cash Flow', b: 'Net margin of ' + (data.profitMargins ? (data.profitMargins * 100).toFixed(1) + '%' : 'N/A') + ' and ROE of ' + (data.roeTTM ? data.roeTTM.toFixed(1) + '%' : 'N/A') + '. Revenue growth: ' + (data.revenueGrowth ? (data.revenueGrowth * 100).toFixed(1) + '% YoY' : 'N/A') + '. Free cash flow (TTM): ' + (data.freeCashflow ? fmtNum(data.freeCashflow) + (data.freeCashflow > 0 ? ' — company generates strong cash after capex.' : ' — cash flow negative, monitor burn rate.') : 'N/A.') },
